...
impl SK {
    ...
    fn gen_leaf_pk(&self, i: Idx) -> Trits {
        let wsk = self.gen_leaf_sk(i as Trint18);
    ...
    MT: TraversableMerkleTree<Trits>,
{
    ...
    fn encode_wotsig(&self, hash: TritSlice, wotsig: TritSliceMut) {
        ...
        let wsk = self.sk.gen_leaf_sk(self.skn() as Trint18);
...
fn parse_skn(t: TritSlice) -> Option<(Trint6, Trint18)> {
    assert!(t.size() == SKN_SIZE);
    let mut ts = Trits::zero(18);
    ...
    if 0 <= height && 0 <= skn && (skn as usize) < max_idx(height as usize) {
        Some((height, skn))
    } else {
        None
    }
...
fn fold_apath(mut skn: Trint18, mut ap: TritSlice, hpk: TritSliceMut) {
    assert!(0 <= skn);
    while !ap.is_empty() {
...
mod test {
    use super::*;
    use crate::mss::{mtcomplete, mttraversal};
    ...
    fn sign_verify_d2_mtcomplete() {
        sign_verify_mt::<mtcomplete::MT<Trits>>();
    }

    #[test]
    fn sign_verify_d2_mttraversal() {
        sign_verify_mt::<mttraversal::MT<Trits>>();
    }
}
