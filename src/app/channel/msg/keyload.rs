...
use crate::ntru;
use crate::pb3::{self, guard, Absorb, Err, Mask, Result};
use crate::prng::PRNG;
use crate::psk::{self};
...
pub fn sizeof(
    psks: &mut dyn ExactSizeIterator<Item = (&psk::PskId, &psk::Psk)>,
    ntrupks: &mut dyn ExactSizeIterator<Item = (&ntru::Pkid, &ntru::PublicKey)>,
) -> usize {
    0
    // join link msgid;
...
pub fn unwrap(
    ...
    ntrusks: &HashMap<ntru::Pkid, ntru::PrivateKey>,
    s: &mut Spongos,
    b: &mut TritSlice,
) -> Result<()> {
...

    {
        ...
        loop {
            ...
            if let Some(ntrusk) = ntrusks.get(&ntrupkid) {
                pb3::ntrukem::unwrap_ntrukem(key.slice_mut(), ntrusk, &mut fork, b)?;
                key_found = true;
            } else {
...
mod test {
    ...
    fn simple() {
...

        {
            ...
            pb3::mssig::squeeze_wrap_mssig(&mss_sk, &mut s, &mut b);
            mss_sk.next();
            assert!(b.size() == 0);
        }
        ...
        // unwrap using psks
        {
            ...
            assert!(r0 == Ok(()));
            let r1 = pb3::mssig::squeeze_unwrap_mssig_verify(mss_sk.public_key(), &mut s, &mut b);
            assert!(r1 == Ok(()));
            assert!(b.size() == 0);
        }
