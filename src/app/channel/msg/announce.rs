...
pub fn wrap(
    ...
    opt_ntru_pk: &Option<ntru::PublicKey>,
    s: &mut Spongos,
    b: &mut TritSliceMut,
) {
    pb3::wrap_absorb_trits(mss_sk.public_key().pk.slice(), s, b);
...
pub fn unwrap(
    s: &mut Spongos,
    b: &mut TritSlice,
) -> Result<(mss::PublicKey, Option<ntru::PublicKey>)> {
...
mod test {
    ...
    fn simple_with_ntrupk() {
        ...
        let d = 1;
        let mut mss_sk = mss::PrivateKey::gen(&prng, mss_nonce.slice(), d);
        ...
        // unwrap
        {
            ...
            assert_eq!(0, b.size());
            assert!(r == Ok((mss_sk.public_key().clone(), Some(ntru_pk))));
        }
    }
    ...
    fn simple_without_ntrupk() {
        ...
        // unwrap
        {
            ...
            let r = msg::announce::unwrap(&mut s, &mut b);
            assert_eq!(0, b.size());
            assert!(r == Ok((mss_sk.public_key().clone(), None)));
        }
    }
}
