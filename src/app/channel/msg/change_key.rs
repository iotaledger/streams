...
use crate::mss;
use crate::pb3::{self, Absorb, Result};
...
pub fn unwrap<F>(lookup_link: F, s: &mut Spongos, b: &mut TritSlice) -> Result<mss::PublicKey>
where
    F: for<'b> Fn(TritSlice<'b>) -> Option<(Spongos, mss::PublicKey)>,
{
...
mod test {
    ...
    fn wrap_unwrap() {
        // secrets, nonces
        let prng_key = trits::Trits::from_str(
        ...
        let d_old = 2;
        let mut mss_sk_old = mss::PrivateKey::gen(&prng, mss_nonce.slice(), d_old);
        let d_new = 3;
        let mut mss_sk_new = mss::PrivateKey::gen(&prng, mss_nonce.slice(), d_new);
        ...
        // unwrap
        {
            ...
            let lookup_link = |m: TritSlice| {
                if m == msgid.slice() {
                    Some((slink.clone(), mss_sk_old.public_key().clone()))
                } else {
                    None
                }
            };
            ...
            assert_eq!(0, b.size());
            assert!(r == Ok(mss_sk_new.public_key().clone()));
        }
    }
}
