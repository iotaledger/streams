...
pub fn wrap_mssig(tag: TritSlice, sk: &mss::PrivateKey, _s: &mut Spongos, b: &mut TritSliceMut) {
...
pub fn unwrap_mssig_recover(
    tag: TritSlice,
    _s: &mut Spongos,
    b: &mut TritSlice,
) -> Result<mss::PublicKey> {
    let mut apk = Trits::zero(mss::PK_SIZE);
    let sig_size = mss::recover(apk.slice_mut(), tag, *b).ok_or(Err::BadValue)?;
...
pub fn squeeze_mss_hash(s: &mut Spongos) -> Trits {
    ...
    s.commit();
    s.squeeze(tag.slice_mut());
    tag
}

/// Squeeze tag and sign it with MSS private key.
pub fn squeeze_wrap_mssig(sk: &mss::PrivateKey, s: &mut Spongos, b: &mut TritSliceMut) {
    let tag = squeeze_mss_hash(s);
    wrap_mssig(tag.slice(), sk, s, b);
}

/// Squeeze tag and recover MSS public key.
pub fn squeeze_unwrap_mssig_recover(s: &mut Spongos, b: &mut TritSlice) -> Result<mss::PublicKey> {
    let tag = squeeze_mss_hash(s);
    unwrap_mssig_recover(tag.slice(), s, b)
}

/// Squeeze tag and verify MSS signature.
pub fn squeeze_unwrap_mssig_verify(
    pk: &mss::PublicKey,
    s: &mut Spongos,
    b: &mut TritSlice,
) -> Result<()> {
...
mod test {
    ...
    fn squeeze_wrap_unwrap() {
        // secrets, nonces
        let prng_key = Trits::from_str(
            "PRNGKEYPRNGKEYPRNGKEYPRNGKEPRNGKEYPRNGKEYPRNGKEYPRNGKEPRNGKEYPRNGKEYPRNGKEYPRNGKE",
        )
        .unwrap();
