    ...
    Self: Sized,
{
    ...
    fn encode(&self, b: &mut TritSliceMut);

    /// Decode the value from the buffer.
    fn decode(&mut self, b: &mut TritSlice) -> Result<()> {
    ...
    /// Decode the value from the buffer.
    fn decode_sized(b: &mut TritSlice) -> Result<Self>;
}
    ...
    T: TrintField,
{
    ...
    fn unwrap_mask_sized(s: &mut Spongos, b: &mut TritSlice) -> Result<Self> {
        ...
        s.decr(b.advance(n), t.slice_mut());
...
mod test {
    ...
    fn trintx_absorb_wrap_unwrap<T>(t: T, sw: &mut Spongos, su: &mut Spongos)
    where
        T: TrintField + Eq + fmt::Debug,
    {
    ...
    fn trintx_mask_wrap_unwrap<T>(t: T, sw: &mut Spongos, su: &mut Spongos)
    where
        T: TrintField + Eq + fmt::Debug,
    {
        let mut buf = Trits::zero(T::sizeof());
