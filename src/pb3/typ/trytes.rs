...
use crate::pb3::typ::size::{sizeof_sizet, Size};
use crate::spongos::Spongos;
use crate::trits::{self, TritSlice, TritSliceMut, Trits};
...
pub fn wrap_absorb_trits(x: TritSlice, s: &mut Spongos, b: &mut TritSliceMut) {
    let n = x.size();
    assert!(n <= b.size());
    let t = b.advance(n);
    x.copy(t);
...
pub fn unwrap_absorb_trits(x: TritSliceMut, s: &mut Spongos, b: &mut TritSlice) -> Result<()> {
    let n = x.size();
...
pub fn unwrap_absorb_n(n: usize, s: &mut Spongos, b: &mut TritSlice) -> Result<()> {
    guard(n <= b.size(), Err::Eof)?;
    s.absorb(b.advance(n));
    Ok(())
}

pub fn wrap_mask_trits(x: TritSlice, s: &mut Spongos, b: &mut TritSliceMut) {
    let n = x.size();
    assert!(n <= b.size());
    let t = b.advance(n);
    s.encr(x, t);
...
pub fn unwrap_mask_trits(x: TritSliceMut, s: &mut Spongos, b: &mut TritSlice) -> Result<()> {
    let n = x.size();
    guard(n <= b.size(), Err::Eof)?;
    let t = b.advance(n);
    s.decr(t, x);
...
pub fn wrap_absorb_trytes(x: &Trits, s: &mut Spongos, b: &mut TritSliceMut) {
    assert!(x.size() % 3 == 0);
    Size(x.size() / 3).wrap_absorb(s, b);
    wrap_absorb_trits(x.slice(), s, b);
}

pub fn unwrap_absorb_trytes(s: &mut Spongos, b: &mut TritSlice) -> Result<Trits> {
    let n = Size::unwrap_absorb_sized(s, b)?;
    let mut x = Trits::zero(n.0 * 3);
    unwrap_absorb_trits(x.slice_mut(), s, b)?;
    Ok(x)
}

pub fn wrap_mask_trytes(x: &Trits, s: &mut Spongos, b: &mut TritSliceMut) {
    assert!(x.size() % 3 == 0);
    Size(x.size() / 3).wrap_mask(s, b);
    wrap_mask_trits(x.slice(), s, b);
}

pub fn unwrap_mask_trytes(s: &mut Spongos, b: &mut TritSlice) -> Result<Trits> {
    let n = Size::unwrap_mask_sized(s, b)?;
    let mut x = Trits::zero(n.0 * 3);
    unwrap_mask_trits(x.slice_mut(), s, b)?;
    Ok(x)
}
...
impl Absorb for NTrytes {
    ...
    fn unwrap_absorb(&mut self, s: &mut Spongos, b: &mut TritSlice) -> Result<()> {
        unwrap_absorb_trits(self.slice_mut(), s, b)
    }

    fn unwrap_absorb_sized(_s: &mut Spongos, _b: &mut TritSlice) -> Result<Self> {
...
impl Mask for NTrytes {
    ...
    fn unwrap_mask(&mut self, s: &mut Spongos, b: &mut TritSlice) -> Result<()> {
        unwrap_mask_trits(self.slice_mut(), s, b)
    }

    fn unwrap_mask_sized(_s: &mut Spongos, _b: &mut TritSlice) -> Result<Self> {
