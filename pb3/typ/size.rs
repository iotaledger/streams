...
fn size_trytes(n: usize) -> usize {
    ...
    // `(27^12 - 1) / 2`.
    const M12: usize = 75047317648499560;
    if n > M12 {
...
impl Size {
    ...
    fn wrap_head(w: &mut dyn Wrap, b: &mut TritMutSlice, d: usize) {
        w.wrap3(b, d as trits::Trint3);
    }

    /// Wrap the representation value trytes.
    fn wrap_tail(w: &mut dyn Wrap, b: &mut TritMutSlice, d: usize, mut n: usize) {
        for _ in 0..d {
    ...
    fn wrap(&self, w: &mut dyn Wrap, b: &mut TritMutSlice) {
    ...
    fn unwrap_head(w: &mut dyn Unwrap, b: &mut TritConstSlice) -> Result<usize> {
    ...
    fn unwrap_tail(w: &mut dyn Unwrap, b: &mut TritConstSlice, mut d: usize) -> Result<usize> {
    ...
    /// Unwrap the value.
    fn unwrap(w: &mut dyn Unwrap, b: &mut TritConstSlice) -> Result<Self> {
...
mod test {
    ...
    fn absorb_wrap_unwrap() {
        ...
        let ns = [0,1,13,14,25,26,27,39,40,81,9840,9841,9842,19683,SIZE_MAX-1,SIZE_MAX];
        for n in ns.iter() {
