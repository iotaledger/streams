...
pub const TYPE: &'static str = "MAM9CHANNEL9SUBSCRIBE";
...
/// * `b` -- output buffer.
///
pub fn wrap(msgid: TritConstSlice, slink: &mut Spongos, prng: &PRNG, nonce: TritConstSlice, sub_pk: &ntru::PublicKey, ch_pk: &ntru::PublicKey, s: &mut Spongos, b: &mut TritMutSlice) {
    pb3::join::wrap_join(msgid, slink, s, b);
    //TODO: fix `nonce`, use `sub_pk`, `ch_pk`.
    let mut unsubscribe_key = Trits::zero(spongos::KEY_SIZE);
    prng.gen(nonce, unsubscribe_key.mut_slice());
...
/// * `b` -- output buffer.
///
pub fn unwrap(lookup_link: impl Fn(TritConstSlice) -> Option<(Spongos, ())>, ch_sk: &ntru::PrivateKey, s: &mut Spongos, b: &mut TritConstSlice) -> Result<ntru::PublicKey> {
    let _ = pb3::join::unwrap_join(lookup_link, s, b)?;
    let mut unsubscribe_key = Trits::zero(spongos::KEY_SIZE);
    pb3::ntrukem::unwrap_ntrukem(unsubscribe_key.mut_slice(), ch_sk, s, b)?;
    s.commit();
...
mod test {
    ...
    fn wrap_unwrap() {
        ...
        let unsubscribe_nonce = trits::Trits::from_str("UNSUBSCRIBE").unwrap();
        ...
        // wrap
        {
            ...
            msg::subscribe::wrap(msgid.slice(), &mut slink, &prng, unsubscribe_nonce.slice(), &sub_ntru_pk, &ch_ntru_pk, &mut s, &mut b);
            assert_eq!(0, b.size());
        }
