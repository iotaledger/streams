...
pub const TYPE: &'static str = "MAM9SIGNEDPACKET";
...
///
/// * `b` -- output buffer.
pub fn wrap(msgid: &MsgId, slink: &mut Spongos, public_payload: &pb3::Trytes, masked_payload: &pb3::Trytes, sk: &mss::PrivateKey, s: &mut Spongos, b: &mut TritMutSlice) {
...
pub fn unwrap_recover(lookup_link: impl Fn(TritConstSlice) -> Option<(Spongos, ())>, s: &mut Spongos, b: &mut TritConstSlice) -> Result<(mss::PublicKey, pb3::Trytes, pb3::Trytes)> {
...
pub fn unwrap_verify(lookup_link: impl Fn(TritConstSlice) -> Option<(Spongos, ())>, mss_pk: &mss::PublicKey, s: &mut Spongos, b: &mut TritConstSlice) -> Result<(pb3::Trytes, pb3::Trytes)> {
    pb3::join::unwrap_join(lookup_link, s, b)?;
    let public_payload = pb3::Trytes::unwrap_absorb_sized(s, b)?;
    let masked_payload = pb3::Trytes::unwrap_mask_sized(s, b)?;
...
mod test {
    ...
    fn wrap_unwrap() {
        ...
        // message
        let n = msg::signed_packet::sizeof(public_payload.size() / 3, masked_payload.size() / 3, &mss_sk);
        ...
        // wrap
        {
            ...
            msg::signed_packet::wrap(&msgid, &mut slink, &public_payload, &masked_payload, &mss_sk, &mut s, &mut b);
            assert_eq!(0, b.size());
        }
        ...
        // unwrap
        {
            ...
            let mut b = buf.slice();
            let mut slink = Spongos::init();
            let r = msg::signed_packet::unwrap_verify(|m| if m == msgid.id.slice() { Some((slink.clone(), ())) } else { None }, &mss_pk, &mut s, &mut b);
            assert_eq!(0, b.size());
