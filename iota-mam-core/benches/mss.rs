...
use iota_mam_core::{
    prng,
    signature::mss::{PrivateKeyMTComplete, PrivateKeyMTTraversal},
    trits::Trits,
};
...
fn troika_benchmark(c: &mut Criterion) {
    ...
    c.bench(
        "Run MSS PrivateKey generation",
        Benchmark::new(&format!("MT complete height={})", mt_height), |b| {
            b.iter(|| {
        ...
        .with_function(&format!("MT traversal height={})", mt_height), |b| {
            b.iter(|| {
                let prng = prng::dbg_init_str("BENCHPRNGKEY");
                let nonce = Trits::from_str("BENCHNONCE").unwrap();
    ...
    c.bench(
        "Run MSS PrivateKey next",
        Benchmark::new(&format!("MT complete height={})", mt_height), move |b| {
            b.iter(|| {
                let mut sk = sk_complete.clone();
                while sk.next() {}
            })
        })
        .with_function(&format!("MT traversal height={})", mt_height), move |b| {
            b.iter(|| {
