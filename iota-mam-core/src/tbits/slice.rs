    ...
    TW: BasicTbitWord,
{
    ...
    unsafe fn is_overlapping(&self, x: &Self) -> bool {
    ...
    pub fn copy(&self, to: &TbitSliceMutT<'a, TW>) {
    ...
    /// Copy tbits into the slice `to` of equal size.
    pub fn copy_min(&self, to: &TbitSliceMutT<'a, TW>) -> usize {
        debug_assert!(unsafe{!self.is_overlapping(&to.as_const())});
...
where
    TW: SpongosTbitWord,
{
    pub fn copy_add(&self, s: &mut TbitSliceMutT<'a, TW>, y: &mut TbitSliceMutT<'a, TW>) {
        assert_eq!(self.size(), y.size());
        assert_eq!(self.size(), s.size());
        unsafe { TW::copy_add(self.size(), self.r.d, self.p, s.r.d, s.p, y.r.d, y.p); }
    }
    pub fn copy_add_min(&self, s: &mut TbitSliceMutT<'a, TW>, y: &mut TbitSliceMutT<'a, TW>) -> usize {
        assert_eq!(self.size(), y.size());
        let n = self.size_min(s.size());
        unsafe { TW::copy_add(n, self.r.d, self.p, s.r.d, s.p, y.r.d, y.p); }
        n
    }

    pub fn copy_sub(&self, s: &mut TbitSliceMutT<'a, TW>, y: &mut TbitSliceMutT<'a, TW>) {
        assert_eq!(self.size(), y.size());
        assert_eq!(self.size(), s.size());
        unsafe { TW::copy_sub(self.size(), self.r.d, self.p, s.r.d, s.p, y.r.d, y.p); }
    }
    pub fn copy_sub_min(&self, s: &mut TbitSliceMutT<'a, TW>, y: &mut TbitSliceMutT<'a, TW>) -> usize {
        assert_eq!(self.size(), y.size());
        let n = self.size_min(s.size());
        unsafe { TW::copy_sub(n, self.r.d, self.p, s.r.d, s.p, y.r.d, y.p); }
        n
    }
}

impl<'a, TW> PartialEq for TbitSliceT<'a, TW>
where
    TW: BasicTbitWord,
{
...
impl<'a, TW> Eq for TbitSliceT<'a, TW> where TW: BasicTbitWord {}

impl<'a, TW> hash::Hash for TbitSliceT<'a, TW>
where
    TW: BasicTbitWord,
    TW::Tbit: hash::Hash,
{
...
/*
impl<'a, TW> fmt::Display for TbitSliceT<'a, TW>
where
    TW: TbitWord + Copy,
{
...
impl<'a, TW> fmt::Debug for TbitSliceT<'a, TW>
where
    TW: BasicTbitWord,
    TW::Tbit: fmt::Display,
{
...
/// MSS, NTRU, Protobuf3).
pub struct TbitSliceMutT<'a, TW: 'a> {
    /// Current slice range.
    pub(crate) r: SliceRange,
    /// Base pointer, always stays constant.
    pub(crate) p: *mut TW,
    ...
    TW: BasicTbitWord,
{
    ...
    pub fn cycle(&mut self, ts: TbitSliceT<'a, TW>) {
    ...
    pub fn into_const(self) -> TbitSliceT<'a, TW> {
        TbitSliceT::<'a, TW> {
            r: self.r,
            p: self.p,
            phantom: std::marker::PhantomData,
    ...
    pub fn with_const<F>(&mut self, f: F) where F: FnOnce(TbitSliceT<'a, TW>) {
        f(TbitSliceT::<'a, TW> {
            r: self.r,
            p: self.p,
            phantom: std::marker::PhantomData,
    ...
    pub unsafe fn as_const(&self) -> TbitSliceT<'a, TW> {
        //TODO: &mut self?
        TbitSliceT::<'a, TW> {
            r: self.r,
            p: self.p,
    ...
    pub fn set_zero(&self) {
        unsafe { TW::set_zero(self.size(), self.r.d, self.p); }
    }
    ...
    pub fn equals(&self, other: &Self) -> bool {
        assert_eq!(self.size(), other.size());
        unsafe { TW::equals(self.size(), self.r.d, self.p as *const TW, other.r.d, other.p as *const TW) }
    }

    /// Compare two slices.
    pub fn equals_min(&self, other: &Self) -> (bool, usize) {
        let n = self.size_min(other.size());
        unsafe { (TW::equals(n, self.r.d, self.p as *const TW, other.r.d, other.p as *const TW), n) }
    }
}
...
where
    TW: SpongosTbitWord,
{
    pub fn swap_add(&mut self, s: &mut Self) {
        assert_eq!(self.size(), s.size());
        unsafe { TW::swap_add(self.size(), self.r.d, self.p, s.r.d, s.p); }
    }
    pub fn swap_add_min(&mut self, s: &mut Self) -> usize {
        let n = self.size_min(s.size());
        unsafe { TW::swap_add(n, self.r.d, self.p, s.r.d, s.p); }
        n
    }

    pub fn swap_sub(&mut self, s: &mut Self) {
        assert_eq!(self.size(), s.size());
        unsafe { TW::swap_sub(self.size(), self.r.d, self.p, s.r.d, s.p); }
    }
    pub fn swap_sub_min(&mut self, s: &mut Self) -> usize {
        let n = self.size_min(s.size());
        unsafe { TW::swap_sub(n, self.r.d, self.p, s.r.d, s.p); }
        n
    }
}

impl<'a, TW: 'a> PartialEq for TbitSliceMutT<'a, TW>
where
    TW: BasicTbitWord,
{
    fn eq(&self, other: &Self) -> bool {
...
impl<'a, TW> Eq for TbitSliceMutT<'a, TW> where TW: BasicTbitWord + Copy {}

/*
impl<'a, TW> fmt::Display for TbitSliceMutT<'a, TW>
where
    TW: TbitWord + Copy,
    TW::Tbit: fmt::Display,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.unsafe_as_const())
    }
}
 */

impl<'a, TW> fmt::Debug for TbitSliceMutT<'a, TW>
where
