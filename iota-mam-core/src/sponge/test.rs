use std::fmt;
use std::str::FromStr;
use crate::tbits::{word::{SpongosTbitWord}, Tbits, trinary::Trit};
use super::spongos::*;
use super::prp::{PRP, Troika};
    ...
    F: PRP<TW> + Default,
{
    ...
    let mut s: Spongos<TW, F>;
    {
        ...
        s.absorb(k.slice());
        s.commit();
        s.squeeze(&mut k.slice_mut());
        s.squeeze(&mut p.slice_mut());
        s.squeeze(&mut x.slice_mut());
    }

    {
        ...
        s.absorb(p.slice());
        s.commit();
        s.encr(x.slice(), &mut y.slice_mut());
        s.commit();
        s.squeeze(&mut t.slice_mut());
    }
...
pub fn encr_decr_n<TW, F>(n: usize)
    where
    TW: SpongosTbitWord,
    TW::Tbit: fmt::Display,
    F: PRP<TW> + Default,
{
    let mut s = Spongos::<TW, F>::init();
    s.absorb_tbits(&Tbits::cycle_str(KEY_SIZE, "KEY"));
    s.commit();
    ...
    let x = Tbits::cycle_str(n, "TEXT");
    {
        ...
        let ex = s.encr_tbits(&x);
        s.commit();
        let tag = s.squeeze_tbits(RATE);

        let dex = s2.decr_tbits(&ex);
        assert_eq!(x, dex);
        s2.commit();
        assert_eq!(tag, s2.squeeze_tbits(RATE));

        let mut x2 = x.clone();
        s3.encr_mut_tbits(&mut x2);
        assert_eq!(ex, x2);
        s3.commit();
        assert_eq!(tag, s3.squeeze_tbits(RATE));

        s4.decr_mut_tbits(&mut x2);
        assert_eq!(x, x2);
        s4.commit();
        assert_eq!(tag, s4.squeeze_tbits(RATE));
    }
}

#[test]
fn encr_decr_troika() {
    encr_decr_n::<Trit, Troika>(27);
    encr_decr_n::<Trit, Troika>(RATE);
    encr_decr_n::<Trit, Troika>(RATE - 28);
    encr_decr_n::<Trit, Troika>(RATE + 28);
    encr_decr_n::<Trit, Troika>(2 * RATE);
}
