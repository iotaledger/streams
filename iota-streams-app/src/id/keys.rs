...
use iota_streams_ddml::{
    command::{Ed25519, sizeof, unwrap, wrap, Commit, Squeeze},
    io,
    types::{HashSig, U64, External, NBytes},
};
use crate::{
    id::Identifier,
    message::{ContentSign, ContentSizeof, ContentVerify}
};
...
#[cfg(feature = "account")]
use iota_streams_core::{
    iota_identity::account::{Account, Command, IdentityId},
};
        ...
        match &id {
            Identifier::EdPubKey(pk) => {
                ...
                let kp = {
                    #[cfg(feature = "use-did")]
                    {
                        let mut kp = KeyPairs { id, did_info: None, sig_kp, ke_kp };
            ...
            Identifier::DID(did) => {
                let mut kp = KeyPairs {
                    ...
                    ke_kp: (x25519::StaticSecret::from([0_u8; 32]), x25519::PublicKey::from([0_u8;32]))
                };
...
impl KeyPairs {
    ...
    pub async fn new_from_account(account: Account, key_fragment: String, did_client: Client) -> Result<KeyPairs> {
        ...
        match account.store().index().await?.get(&IdentityId::from(1)) {
            Some(id) => {
                ...
                let command = Command::create_method().fragment(&key_fragment).keypair(did_sig_kp).finish()?;
                account.update_identity(doc.id(), command).await?;
                let did = doc.id();
                let mut prepend = String::from("#");
                prepend.push_str(&key_fragment);
                let did_info = DIDInfo { did: Some(did.clone()), key_fragment: prepend, did_client };

                Ok( Self { id: did.into(), did_info: Some(did_info), sig_kp, ke_kp } )
            },
    ...
    pub async fn new_from_did<F: PRP>(
        ...
        let mut prepend = String::from("#");
        prepend.push_str(&key_fragment);
        let did_info = DIDInfo { did: Some(doc.id().clone()), key_fragment: prepend, did_client };
    ...
    // TODO: This currently defaults to mainnet with chrysalis-nodes, we need to figure out a reasonable way to go about making this accessible from the user api
    pub async fn make_default_did_info(&mut self) -> Result<()> {
        let did_client = Client::builder().network(Network::Mainnet).primary_node("https://chrysalis-nodes.iota.org", None, None)?.build().await?;
        let info = DIDInfo { did: None, key_fragment: "".to_string(), did_client };
...
impl KeyPairs {
            ...
            Some(info) => {
                match info.did.as_ref() {
                    Some(did) => {
                        ...
                        data.set_signature(
                            Signature::new(JcsEd25519::<DIDEd25519>::NAME, method.as_str())
                        );
                        ...
                        write.set_value(
                            SignatureValue::Signature(encode_b58(&signature.to_bytes()))
                        );
    ...
    fn set_did(&mut self, did: IotaDID) -> Result<()> {
        match self.did_info.as_mut() {
            Some(info) => {info.did = Some(did); Ok(())},
        ...
        #[cfg(feature = "use-did")]
            {
                if let Some(info) = self.did_info.as_ref() {
                    if let Some(did) = info.did.as_ref() {
                        ...
                        let mut wrapper = DataWrapper { data: prehashed, signature: None };
                    ...
                    1 => {
                        match self.did_info.as_ref() {
                            Some(_info) => {
                                ...
                                // Place hash in data wrapper and sign it
                                let wrapper = DataWrapper { data: prehashed, signature: Some(signature) };
...
pub async fn create_identity(url: &str, network: Network) -> Result<(String, DIDKeyPair, Client)> {
    match Client::builder().network(network).primary_node(url, None, None)?.build().await {
        Ok(client) => {
