...
fn make_tx(address: &Trits, tag: &Trits, msg: &Trits, timestamp: i64) -> iota_model::Transaction {
    debug_assert_eq!(243, address.size());
    debug_assert_eq!(81, tag.size());
    debug_assert_eq!(6561, msg.size());

    let mut tx = make_empty_tx();
    tx.address = address.to_str();
    tx.tag = tag.to_str();
    tx.signature_fragments = msg.to_str();
    tx.timestamp = timestamp;
    tx
}

fn pad_trits(n: usize, s: TritSlice) -> Trits {
    if n > s.size() {
...
/// As MAM Message are packed into a bundle, and different bundles can have the same hash
...
/// This leads that it may not be possible to MAM Messages from bundle hash only.
/// So this function also takes into account `address` and `tag` fields.
/// As MAM Messages can have the same message id (ie. `tag`) it is advised that MAM Message
/// bundles have distinct nonces and/or timestamps.
pub fn msg_to_bundle(msg: &TrinaryMessage<TangleAddress>, timestamp: i64) -> iota_model::Bundle {
    let mut bundle = iota_model::Bundle::new(make_txs(
        msg.link().appinst.trits(),
        msg.link().msgid.trits(),
        &msg.body,
        timestamp,
    ));
...
/// Reconstruct MAM Message from bundle. The input bundle is not checked (for validity of
/// the hash, consistency of indices, etc.). Checked bundles are returned by `bundles_from_trytes`.
pub fn msg_from_bundle(bundle: &iota_model::Bundle) -> TrinaryMessage<TangleAddress> {
    let tx = &bundle[0];
    let appinst = AppInst {
        id: NTrytes(Trits::from_str(&tx.address).unwrap()),
    };
    let msgid = MsgId {
        id: NTrytes(Trits::from_str(&tx.tag).unwrap()),
    };
    let mut body = Trits::zero(0);
    for tx in bundle.iter() {
        body += &Trits::from_str(&tx.signature_fragments).unwrap();
    }
    TrinaryMessage::new(TangleAddress { appinst, msgid }, body)
}

#[cfg(test)]
#[test]
fn bundle_from_to_trytes() {
    let appinst = AppInst {
        id: NTrytes(Trits::cycle_str(APPINST_SIZE, "A")),
    };
    // A,M,[D,E]
    let m1 = {
        let msgid = MsgId {
            id: NTrytes(Trits::cycle_str(MSGID_SIZE, "M")),
        };
        let body = &Trits::cycle_str(6561, "D") + &Trits::cycle_str(6561, "E");
        TrinaryMessage::new(TangleAddress::new(appinst.clone(), msgid), body)
    };
    // A,N,[F,G]
    let m2 = {
        let msgid = MsgId {
            id: NTrytes(Trits::cycle_str(MSGID_SIZE, "N")),
        };
        let body = &Trits::cycle_str(6561, "F") + &Trits::cycle_str(6561, "G");
        TrinaryMessage::new(TangleAddress::new(appinst.clone(), msgid), body)
    };
...
impl<'a> Transport<TangleAddress> for iota_client::Client<'a> {
    /// Send a MAM message over the Tangle with the current timestamp and default SendTrytesOptions.
    fn send_message(&mut self, msg: &TrinaryMessage<TangleAddress>) -> Fallible<()> {
    ...
    fn recv_message(
        &mut self,
        link: &TangleAddress,
    ) -> Fallible<Vec<TrinaryMessage<TangleAddress>>> {
