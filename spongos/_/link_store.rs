...
struct EmptyLinkStore<F, Link, Info>(core::marker::PhantomData<(F, Link, Info)>);
...
struct SingleLinkStore<F: PRP, Link, Info>(Link, (Inner<F>, Info));

impl<F: PRP, Link, Info> SingleLinkStore<F, Link, Info> {
    fn link(&self) -> &Link {
        &self.0
    }
    fn spongos(&self) -> &Inner<F> {
        &(self.1).0
    }
    fn info(&self) -> &Info {
        &(self.1).1
...
struct DefaultLinkStore<F: PRP, Link, Info> {
