...
use iota_mam_core::trits::{word::BasicTritWord, DefaultTritWord, TritSlice, TritSliceMut, Trits};
...
pub(crate) trait Unwrap {
    ...
    fn unwrapn(&mut self, trits: TritSliceMut) -> Fallible<&mut Self>;
}

/// Helper function for unwrapping (decoding/absorbing) size values.
pub(crate) fn unwrap_size<'a, Ctx: Unwrap>(
    ctx: &'a mut Ctx,
    size: &mut Size,
...
fn unwrap_absorb_trint3<'a, IS: io::IStream>(
    ctx: &'a mut AbsorbContext<IS>,
    trint3: &mut Trint3,
) -> Fallible<&'a mut AbsorbContext<IS>> where
{
    ctx.unwrap3(trint3)
}
fn unwrap_absorb_size<'a, IS: io::IStream>(
    ctx: &'a mut AbsorbContext<IS>,
    size: &mut Size,
) -> Fallible<&'a mut AbsorbContext<IS>> where
{
...
fn unwrap_absorb_trits<'a, IS: io::IStream>(
    ctx: &'a mut AbsorbContext<IS>,
    trits: TritSliceMut,
) -> Fallible<&'a mut AbsorbContext<IS>> where
{
...
impl<'a, IS: io::IStream> Absorb<&'a mut mss::PublicKey> for Context<IS> {
    fn absorb(&mut self, pk: &'a mut mss::PublicKey) -> Fallible<&mut Self> {
        ensure!(pk.trits().size() == mss::PK_SIZE);
        Ok(unwrap_absorb_trits(self.as_mut(), pk.trits_mut().slice_mut())?.as_mut())
    }
}

impl<'a, IS: io::IStream> Absorb<&'a mut ntru::PublicKey> for Context<IS> {
    fn absorb(&mut self, pk: &'a mut ntru::PublicKey) -> Fallible<&mut Self> {
        ensure!(pk.trits().size() == ntru::PK_SIZE);
        unwrap_absorb_trits(self.as_mut(), pk.trits_mut().slice_mut())?;
...
fn wrap_absorb_external_trint3<'a, IS: io::IStream>(
    ctx: &'a mut AbsorbExternalContext<IS>,
    trint3: Trint3,
) -> Fallible<&'a mut AbsorbExternalContext<IS>> where
{
    ctx.wrap3(trint3)
}
fn wrap_absorb_external_size<'a, IS: io::IStream>(
    ctx: &'a mut AbsorbExternalContext<IS>,
    size: Size,
) -> Fallible<&'a mut AbsorbExternalContext<IS>> where
{
...
fn wrap_absorb_external_trits<'a, IS: io::IStream>(
    ctx: &'a mut AbsorbExternalContext<IS>,
    trits: TritSlice,
) -> Fallible<&'a mut AbsorbExternalContext<IS>> where
{
...
impl<'a, T: 'a, IS: io::IStream> Absorb<&'a External<T>> for Context<IS>
where
...
impl<'a, IS: io::IStream> Absorb<External<&'a Trint3>> for Context<IS> {
...
impl<'a, IS> Squeeze<&'a mut External<NTrytes>> for Context<IS> {
    fn squeeze(&mut self, val: &'a mut External<NTrytes>) -> Fallible<&mut Self> {
...
fn unwrap_mask_trint3<'a, IS: io::IStream>(
    ctx: &'a mut MaskContext<IS>,
    trint3: &mut Trint3,
) -> Fallible<&'a mut MaskContext<IS>> where
{
    ctx.unwrap3(trint3)
}
fn unwrap_mask_size<'a, IS: io::IStream>(
    ctx: &'a mut MaskContext<IS>,
    size: &mut Size,
) -> Fallible<&'a mut MaskContext<IS>> where
{
...
fn unwrap_mask_trits<'a, IS: io::IStream>(
    ctx: &'a mut MaskContext<IS>,
    trits: TritSliceMut,
) -> Fallible<&'a mut MaskContext<IS>> where
{
...
impl<'a, IS: io::IStream> Mask<&'a mut ntru::PublicKey> for Context<IS> {
    fn mask(&mut self, ntru_pk: &'a mut ntru::PublicKey) -> Fallible<&mut Self> {
        ensure!(ntru_pk.trits().size() == ntru::PK_SIZE);
        unwrap_mask_trits(self.as_mut(), ntru_pk.trits_mut().slice_mut())?;
        ensure!(ntru_pk.validate(), "Unmasked NTRU public key is not valid.");
        Ok(self)
    }
}

impl<'a, IS: io::IStream> Mask<&'a mut mss::PublicKey> for Context<IS> {
    fn mask(&mut self, mss_pk: &'a mut mss::PublicKey) -> Fallible<&mut Self> {
        ensure!(mss_pk.trits().size() == mss::PK_SIZE);
        Ok(unwrap_mask_trits(self.as_mut(), mss_pk.trits_mut().slice_mut())?.as_mut())
    }
}
...
fn unwrap_skip_trint3<'a, IS: io::IStream>(
    ctx: &'a mut SkipContext<IS>,
    trint3: &mut Trint3,
) -> Fallible<&'a mut SkipContext<IS>> where
{
    ctx.unwrap3(trint3)
}
fn unwrap_skip_size<'a, IS: io::IStream>(
    ctx: &'a mut SkipContext<IS>,
    size: &mut Size,
) -> Fallible<&'a mut SkipContext<IS>> where
{
...
fn unwrap_skip_trits<'a, IS: io::IStream>(
    ctx: &'a mut SkipContext<IS>,
    trits: TritSliceMut,
) -> Fallible<&'a mut SkipContext<IS>> where
{
...
impl<'a, IS: io::IStream> Mssig<&'a mut mss::PublicKey, MssHashSig> for Context<IS> {
    fn mssig(&mut self, apk: &'a mut mss::PublicKey, _hash: MssHashSig) -> Fallible<&mut Self> {
...
impl<'a, IS: io::IStream> Mssig<&'a mss::PublicKey, MssHashSig> for Context<IS> {
    fn mssig(&mut self, pk: &'a mss::PublicKey, _hash: MssHashSig) -> Fallible<&mut Self> {
        let mut hash = External(NTrytes(Trits::zero(mss::HASH_SIZE)));
...
impl<'a, IS: io::IStream> Ntrukem<&'a ntru::PrivateKey, &'a mut NTrytes> for Context<IS> {
    fn ntrukem(
        &mut self,
        sk: &'a ntru::PrivateKey,
        secret: &'a mut NTrytes,
    ) -> Fallible<&mut Self> {
        ...
        let ekey_slice = self.stream.try_advance(ntru::EKEY_SIZE)?;
            ...
            sk.decr_with_s(&mut self.spongos, ekey_slice, (secret.0).slice_mut()),
...
impl<F, IS: io::IStream> Fork<F> for Context<IS>
where
...
impl<F, IS: io::IStream> Repeated<Size, F> for Context<IS>
where
    F: for<'a> FnMut(&'a mut Self) -> Fallible<&'a mut Self>,
{
    fn repeated(&mut self, n: Size, mut value_handle: F) -> Fallible<&mut Self> {
        for _ in 0..(n.0) {
            value_handle(self)?;
        }
        Ok(self)
...
impl<'a, T: 'a + AbsorbFallback, IS: io::IStream> Absorb<&'a mut T> for Context<IS> {
    fn absorb(&mut self, val: &'a mut T) -> Fallible<&mut Self> {
...
impl<'a, T: 'a + SkipFallback, IS: io::IStream> Skip<&'a mut T> for Context<IS> {
    fn skip(&mut self, val: &'a mut T) -> Fallible<&mut Self> {
        val.unwrap_skip(self)?;
        Ok(self)
    }
}

impl<'a, L: SkipFallback, S: LinkStore<L>, IS: io::IStream> Join<&'a mut L, &S> for Context<IS> {
...
impl<IS: io::IStream> Dump for Context<IS> {
