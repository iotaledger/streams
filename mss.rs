...
const SKN_TREE_DEPTH_SIZE: usize = 4;
...
const SKN_SIZE: usize = SKN_TREE_DEPTH_SIZE + SKN_KEY_NUMBER_SIZE;

/// MSS authentication path size of height `d`.
fn apath_size(d: usize) -> usize { wots::PUBLIC_KEY_SIZE * d }

/// MSS signature size with a tree of height `d`.
pub fn sig_size(d: usize) -> usize { SKN_SIZE + wots::SIGNATURE_SIZE + apath_size(d) }
...
impl<TW> PrivateKey<TW> where TW: TritWord + Copy {
    ...
    /// Encode current WOTS secret key number.
    fn skn(&self, skn: TritMutSlice<TW>) {
        assert!(skn.size() == SKN_SIZE);
        let mut ts = Trits::<TW>::zero(18);

        ts.mut_slice().put6(self.height as Trint6);
        ts.slice().take(4).copy(skn.take(4));

        ts.mut_slice().put18(self.skn as Trint18);
        ts.slice().take(14).copy(skn.drop(4));
    }
...
fn parse_skn<TW>(d: &mut Trint6, skn: &mut Trint18, t: TritConstSlice<TW>) -> bool where TW: TritWord + Copy {
    assert!(t.size() == 18);
    let mut ts = Trits::<TW>::zero(18);
    ...
    *skn = ts.slice().get18();
    ...
    0 <= *d && 0 <= *skn && (*skn as usize) < max_skn(*d as usize)
}
...
pub fn verify<TW>(pk: TritConstSlice<TW>, hash: TritConstSlice<TW>, mut sig: TritConstSlice<TW>) -> bool where TW: TritWord + Copy {
    ...
    if !parse_skn(&mut d, &mut skn, sig.take(18)) { return false; }
    sig = sig.drop(18);

    if sig.size() != sig_size(d as usize) - SKN_SIZE { return false; }
